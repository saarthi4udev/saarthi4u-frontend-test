...
export default async function NewsPage({ searchParams }: Props) {
  ...
  const response = await getAllNews(currentPage, 4);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {newsItems.length === 0 ? (
            ...
            newsItems.map((item) => (
              ...
              >
                ...
                {/* TITLE */}
                <h2 className="text-xl font-bold text-primary dark:text-white mt-2">
                  {item.title}
                </h2>
                ...
                <Link
                  href={`/news/${item.slug}`}
                  className="mt-5 inline-block text-sm font-semibold text-primary dark:text-secondary"
                >
                  Read more →
                </Link>
              </article>
        ...
        <div className="mt-10 flex justify-center items-center gap-4">
          ...
          <Link
            ...
            className={`px-4 py-2 rounded-full border border-primary/20 text-sm font-semibold ${
              currentPage === 1
